...
g1 <- ggplot(df, aes(x=date, y=new_cases_smoothed_per_million, color=class, group=train.test)) + 
  geom_point(aes(shape=train.test)) + labs(title = 'New COVID-19 cases per million', y = "new cases", x = "date", shape = 'Test/train\n', color = "Class\n") + 
...
data <- train.set[, c('class',"new_cases_smoothed_per_million", "new_deaths_smoothed_per_million", 
               'new_vaccinations_smoothed_per_million', 'new_tests_smoothed_per_thousand',
...
df$model.predictions <- predict(model1, df[,c('class',"new_cases_smoothed_per_million", "new_deaths_smoothed_per_million", 
                                              'new_vaccinations_smoothed_per_million', 'new_tests_smoothed_per_thousand',
                                              'new_people_vaccinated_smoothed_per_hundred','new_cases_diff_now','new_deaths_diff', 
                                              'new_vaccinations_diff', 'new_tests_diff', 'new_people_vaccinated_diff', 'tests_per_case')])
...
g2 <- ggplot(df, aes(x=date, y=new_cases_smoothed_per_million, color=model.predictions_diff, group=train.test)) + 
  geom_point(aes(shape=train.test)) + labs(title = 'New COVID-19 cases per million', y = "new cases", x = "date", color = "Prediction\n", shape = 'Test/train\n') + 
  scale_color_manual(labels = c("correct", "wrong"), values = c("blue", "red"))
print(g2)
